...
public class BinarySearchTree<E> {
	...
	private final Comparator<? super E> comparator; // <? super E> 제네릭 타입 E와 E의 모든 부모 클래스를 받아들임

	public BinarySearchTree() {
		this(null);
	} // 타입에 비교 정의 객체가 포함되어 있는 경우에 사용
	public BinarySearchTree(Comparator<? super E> comparator) {
		this.comparator = comparator;
		this.root = null;
		this.size = 0;
	} // 타입에 비교 정의 객체가 미구현 타입인 경우에는 Comparator 인터페이스를 구현한 객체가 전달되어야 함
	
	// 노드 추가를 위한 addUsingXXX()메소드는 반환 타입이 E 타입
	// 중복 원소를 삽입 할 경우 해당 value를 반환하고 삽입이 완료되고 null을 반환함
	
	public boolean add(E value) {
		if(comparator == null) return addUsingComparable(value) == null;
		else return addUsingComparator(value, comparator) == null;
	}
	...
	private E addUsingComparable(E value) {
		...
		//Current가 null, 즉 root가 null이면 root에 새노드 연결 후
		if (current == null) {
		...
		// 기존 트리에 노드가 1개 이상 있는 경우 방향을 결정
		do {
			// 삽입 위치를 유지하는 변수 update
			currentParent = current;
			//새로운 노드의 값과 현재 노드의 값을 비교
			compResult = compValue.compareTo(current.value);
			//비교 결과에 따라 방향 결정, 동일하면 value 그대로 return
			if(compResult < 0) { // 새로운 노드 value가 현재 값보다 작다는 거임
				current = current.left;
			} else if(compResult > 0) { // 새로운 노드 value가 현재 값보다 크다는 거임
		...
		Node<E> newNode = new Node<>(value, currentParent); // 부모노드 링크에 값을 전달하는 예시
		//Node<E> newNode = new Node<>(value);
		if(compResult < 0) currentParent.left = newNode;
	...
	//Comparator를 이용한 add 메소드 - Comparable 이용한 메소드와 비교 / 메소드만 다르고 다른 절차는 동일
	private E addUsingComparator(E value, Comparator<? super E> comp) {
		...
		do {
			currentParent = current;
			//새로운 노드의 값과 현재 노드의 값을 비교 - compare(obj, obj2)
	...
	public E remove(E value) {
		if(comparator == null) return removeUsingComparable(value);
	...
	private E removeUsingComparable(E value) {
		...
		do { // 삭제할 노드 찾는 역할, current.value와 compValue가 동일하면 삭제 노드를 찾게 됨, 찾으면 break
			// current가 null이면 해당 value의 node가 없음
			resComp = compValue.compareTo(current.value);
			if(resComp == 0) break; //삭제할 노드 찾음 -> 반복x
		...
		if(current == null) return null; // 찾는 값이 없음
		
		if(currentParent == null) {//찾는 값이 루트에 있음
			deleteNode(current); // deleteNode->node 삭제 후 재배치 역할
			size--;
			return value;
		}
		
		if(hasLeft) { // currentParent의 왼쪽 노드가 삭제됨
			currentParent.left = deleteNode(current);
		} else { // currentParent의 오른쪽 노드가 삭제됨
	...
	private Node<E> deleteNode(Node<E> removeNode) {
		if(removeNode != null) {
			if(removeNode.left == null && removeNode.right == null) { // 삭제하려는 노드가 단말(자손 없는 자식)노드 일때
				if(removeNode==root) root = null;
				else removeNode=null;
				return null;
			}
			if(removeNode.left != null && removeNode.right != null) { // 삭제하려는 노드가 양쪽에 자식을 끼고 있을때
				// 삭제하려는 node의 값을 다른값으로 대체
				// 대체값 결정 방법
				// 왼쪽 서브트리에서 제일 큰값의 노드로 대체
				//or 오른쪽 서브트리에서 제일 작은값의 노드로 대체
				Node<E> replacement = getSuccessorAndUnlink(removeNode);
				removeNode.value = replacement.value;
			} else if (removeNode.left != null ) { // 왼쪽만
				if (removeNode == root) {
					removeNode = removeNode.left;
					root = removeNode;
				} else removeNode = removeNode.left; // 삭제할 노드의 왼쪽 자식 노드로 업데이트
			} else { // 오른쪽만
				if (removeNode == root) {
					removeNode = removeNode.right;
					root = removeNode;
				} else removeNode = removeNode.right; // 삭제할 노드의 오른쪽 자식 노드로 업데이트
			}
		}
		return removeNode; // 삭제된 자리 대체할 노드의 참조
	}
	// 삭제할 자리를 대체할 노드(후계자)를 찾는 메소드
	...
	private Node<E> getSuccessorAndUnlink(Node<E> node) {
		...
		// 반복문 종료 후 current에 가장 작은 값의 노드가 참조됨, current의 왼쪽 노드 없음
	...
	// 이진 탐색 트리가 비어있는지 판단하는 메서드
	public boolean isEmpty() {
		return size() == 0;
	}
	
	// 이진 탐색 트리 내에 찾고자 하는 객체가 존재하는지를 판단하는 메서드
	public boolean contains(Object o) {
		if(comparator == null) return containsUsingComparable(o);
		return containsUsingComparator(o, comparator);
	}
	
	// 이진트리는 대소관계로 방향이 결정되므로 비교 결과에 따라 왼쪽 or 오른쪽 노드로 이동하며 비교
	private boolean containsUsingComparable(Object o) {
		...
		while(node != null) { // 객체가 찾아지면 true 리턴
			...
			else return true; //(res = 0)
		}
		// 반복문이 종료되어도 return이 안되었다면 객체가 없음을 의미함
		return false;
	}
	
	private boolean containsUsingComparator(Object o, Comparator<? super E> comparator2) {
		return false;
	}
