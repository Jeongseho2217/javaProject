...
public class HashMapWordCount {
	...
	public static void main(String[] args) {
		...
		for(int i=0; i<sentences.length; i++) {
			textArray[1] = sentences[i].trim().split(" ");
		...
		for(int i=0; i<sentences.length; i++) {
			for(int j=0; j<textArray[i].length; j++) {
				if(!textArray[i][j].isEmpty()) {
					String word = textArray[i][j].replaceAll("[^a-zA-Z]", "").toLowerCase(); // 한 단어 추출 영문자와 대소문자 통일
					
					// 단어를 key로 빈도수를 value 저장
					// map.getOrDefault(key, 기본값) -> key가 map에 있으면 값을 반환 없으면 기본값 반환
					// map의 key가 있을 때 put은 value update
					// 기존값 반환받아 1 증가 후 update, 키가 없는 경우 0ㅇ을 반환받아 1 증가 후 put
					wordCount.put(word, wordCount.getOrDefault(word, 0) + 1);
				}
			}
		}
		
		// map 출력 : 키 값을 세트로 추출후 키 : 값 : 전체 대비 백분율
		// 전체 대비 백분율 위해서 값의 합을 구함
		...
		for(Map.Entry<String, Integer> entry : wordCount.entrySet()) {
			System.out.printf("%s : %d : %.2f%%%", entry.getKey(), entry.getValue(), (((double)entry.getValue()/size)*100));
		}
		
		// 단어 빈도에 따른 통계 출력 : 전체 단어 수: Value의 합/ 고유 단어수 : 키의 수 -> size()엔트리 세트 수를 반환
		// 빈도가 가장 높은 / 낮은 단어
