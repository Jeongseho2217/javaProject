...
<!-- 영역이 page이므로 변수처럼 사용 가능 value에 el 표현 사용 가능 -->
		...
		<table border=1 align="center">
			...
			<c:choose>
				...
				<c:when test="${pwd==null}">
					<tr align="center"><td colspan="5">비번이 없습니다.</td></tr>
				</c:when>
				<c:when test="${name==null}">
					<tr align="center"><td colspan="5">이름이 없습니다.</td></tr>
				</c:when>
				<c:when test="${empty age}"> <%-- 변수가 존재하지 않거나 변수가 있더라도 값이 없는 경우 --%>
				...
				<c:otherwise>
					<tr align="center">
						<td>${id}</td>
						<!-- 여기에 html 주석 포함될 수 있음 html 영역으로 전환됨 -->
						<td>${pwd}</td>
						<td>${name}</td>
						<td>${age}</td>
						<td>${height}</td>
