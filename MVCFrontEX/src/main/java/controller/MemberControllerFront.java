...
public class MemberControllerFront extends HttpServlet {
	...
	private void doHandle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String nextPage = null; // forward 시킬 location을 저장하는 변수
		request.setCharacterEncoding("utf-8");
		response.setContentType("text/html;charset=utf-8");
		String action = request.getPathInfo(); // 서블릿에 전달된 최종 경로만 반환
		System.out.println("action : " + action); // 해당 함수에 전달되는 경로는 /member 뒤의 경로만 반환
		// /member/list로 요청하면 action에는 /list를 저장하게 됨
		// /member/listMembers.do 또는 /member -> 회원목록
		if(action == null || action.equals("/listMembers.do")) {
			List<MemberVO> membersList = memberDAO.listMembers();
			System.out.println("서블릿에서 조회된 회원 수: " + membersList.size());
			...
			response.sendRedirect(request.getContextPath() + "/member/listMembers.do?msg=addMember");
			return;
//			nextPage = "/view/listMembers.do";
